...
def draw2(step_list, item_list_train, list1, list2, chart_name, y_label, y_lim=-1, enable_vertical_reference_lines=False, draw_avg=False):
    ...
    '''
    cl = next(colors)
    plt.plot(range(len(list1)), list1, label="Per Episode Average Reward", color=cl, linestyle=next(dashes_styles))
    plt.fill_between(range(len(list1)), np.array(list1) + list2, np.array(list1) - list2, alpha=0.25, color=cl, rasterized=True)
    ...
    final_output = "../outputs/2way-single-roundabout_{}/charts/test/{}".format(EXPERIMENT_NO, file_name)
...
def print_q_table(q_table_param):
    for inner_agent_id in q_table_param.keys():
        ...
        with open('../outputs/2way-single-roundabout/q_table_{}_agent{}_alpha{}_gamma{}_eps{}_decay{}_reward{}_run{}.txt'.format(experiment_time, agent_id, args.alpha, args.gamma, args.epsilon, args.decay, args.reward, run), 'w') as convert_file:
...
if __name__ == '__main__':
    ...
    out_csv = '../outputs/2way-single-roundabout_{}/test/{}_test_{}_alpha{}_gamma{}_eps{}_decay{}_reward{}'.format(EXPERIMENT_NO, EXPERIMENT_NO, experiment_time,
                                                                                                     args.alpha,
                                                                                                     args.gamma,
                                                                                                     args.epsilon,
                                                                                                     args.decay,
    ...
    final_log_file = "../outputs/2way-single-roundabout_{}/log/{}-metrics-test.log".format(EXPERIMENT_NO,
                                                                                             EXPERIMENT_NO)
    ...
    #seed = 80
    seed = args.seed
    print("Run: {} for Testing experiment: {} - route: {} - first seed: {} - alpha: {} - gamma: {} - epsilon: {} - reward: {}".format(args.run_no, EXPERIMENT_NO, args.route, seed, args.alpha, args.gamma, args.epsilon, args.reward))
    logging.info("Run: {} for Testing experiment: {} - route: {} - first seed: {} - alpha: {} - gamma: {} - epsilon: {} - reward: {}".format(args.run_no, EXPERIMENT_NO, args.route, seed, args.alpha, args.gamma, args.epsilon, args.reward))
    ...
    for run in range(1, 2):
        ...
        while not done['__all__']:
            if state_ not in q_table:
                #print("state_ does not exist: {}".format(state_))
                if state_ not in missing_states:
                    missing_states[state_] = 0

                #missing_state_count += 1
                missing_states[state_] += 1 # lane indicaes began at index 2
                #actions = {ts: 0}
                actions = {ts: np.argmax(state_[2:])}
                #print("missing {} - selected {}".format(state_, actions))
            else:
                actions = {ts: np.argmax(q_table[state_])}
    ...
    logging.info("Total Distinct Missing States: {}".format(len(missing_states.keys())))
    logging.info("Total Non-Distinct Missing States: {}".format(sum(missing_states.values())))

    for key in missing_states.keys():
        logging.info("{}: {}".format(key, missing_states[key]))
    #print("********************")
    logging.info("********************")
    ...
    ##draw2(list(), episode_rewards, list1=result_list, list2=np.mean(std_list), chart_name='METRIC: Testing Episode Rewards', y_label="Seconds", draw_avg=False)

    #draw2(list(), J0_stopped_list, chart_name='METRIC: Testing Episode J0 Stopped', y_label="# of cars", draw_avg=False)
    ##draw2(list(), J0_stopped_list, list1=result_stopped_list, list2=np.mean(std_stopped_list), chart_name='METRIC: Training Episode J0 Stopped', y_label="# of cars", draw_avg=False)

    #draw2(list(), J0_stopped_cross_check_list, chart_name='METRIC: Training Episode J0 Stopped', y_label="# of cars", draw_avg=False)
    ##draw2(list(), J0_stopped_cross_check_list, list1=result_stopped_cross_check_list, list2=np.mean(std_stopped_cross_check_list), chart_name='METRIC: Training Episode J0 Stopped Cross Check', y_label="# of cars", draw_avg=False)

    #draw2(list(), J0_acc_wt_list, chart_name='METRIC: Testing Episode J0 Acc. Waiting Time', y_label="Seconds", draw_avg=False)
    ##draw2(list(), J0_acc_wt_list, list1=result_wt_list, list2=np.mean(std_wt_list), chart_name='METRIC: Training Episode J0 Acc. Waiting Time', y_label="Seconds", draw_avg=False)

    #draw2(list(), J0_avg_speed_list, chart_name='METRIC: Testing Episode J0 Avg. Speed', y_label="Seconds", draw_avg=False)
    ##draw2(list(), J0_avg_speed_list, list1=result_avg_speed_list, list2=np.mean(std_avg_speed_list), chart_name='METRIC: Training Episode J0 Avg. Speed List', y_label="Seconds", draw_avg=False)

    # draw2(list(), J0_avg_speed_2_list, chart_name='METRIC: Testing Episode J0 Avg. Speed 2', y_label="Seconds", draw_avg=False)
    ##draw2(list(), J0_avg_speed_2_list, list1=result_avg_speed_2_list, list2=np.mean(std_avg_speed_2_list), chart_name='METRIC: Training Episode J0 Avg. Speed 2 List', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_avg_speed_cross_check_list, list1=result_avg_speed_cross_check_list, list2=np.mean(std_avg_speed_cross_check_list), chart_name='METRIC: Training Episode J0 Avg. Speed Cross Check List', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_co2_emission_tl_list, list1=result_average_co2_emission_tl_list, list2=np.mean(std_average_co2_emission_tl_list), chart_name='METRIC: Training Episode J0 Avg. CO2 Emission TL', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_co_emission_tl_list, list1=result_average_co_emission_tl_list, list2=np.mean(std_average_co_emission_tl_list), chart_name='METRIC: Training Episode J0 Avg. CO Emission TL', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_fuel_consumption_tl_list, list1=result_average_fuel_consumption_tl_list, list2=np.mean(std_average_fuel_consumption_tl_list), chart_name='METRIC: Training Episode J0 Avg. Fuel Consumption TL', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_noise_emission_tl_list, list1=result_average_noise_emission_tl_list, list2=np.mean(std_average_noise_emission_tl_list), chart_name='METRIC: Training Episode J0 Avg. Noise Emission TL', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_co2_emission_list, list1=result_average_co2_emission_list, list2=np.mean(std_average_co2_emission_list), chart_name='METRIC: Training Episode J0 Avg. CO2 Emission', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_co_emission_list, list1=result_average_co_emission_list, list2=np.mean(std_average_co_emission_list), chart_name='METRIC: Training Episode J0 Avg. CO Emission', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_fuel_consumption_list, list1=result_average_fuel_consumption_list, list2=np.mean(std_average_fuel_consumption_list), chart_name='METRIC: Training Episode J0 Avg. Fuel Consumption', y_label="Seconds", draw_avg=False)
    ...
    ##draw2(list(), J0_average_noise_emission_list, list1=result_average_noise_emission_list, list2=np.mean(std_average_noise_emission_list), chart_name='METRIC: Training Episode J0 Avg. Noise Emission', y_label="Seconds", draw_avg=False)
