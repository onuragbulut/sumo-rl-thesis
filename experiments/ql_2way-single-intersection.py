...
def draw2(step_list, item_list_train, chart_name, y_label, y_lim=-1, enable_vertical_reference_lines=False, draw_avg=False):
    ...
    final_output = "../outputs/2way-single-roundabout_{}/charts/train/{}".format(EXPERIMENT_NO, file_name)
...
if __name__ == '__main__':
    ...
    out_csv = '../outputs/2way-single-roundabout_{}/train/{}_train_{}_alpha{}_gamma{}_eps{}_decay{}_reward{}'.format(EXPERIMENT_NO, EXPERIMENT_NO, experiment_time,
                                                                                                     args.alpha,
                                                                                                     args.gamma,
                                                                                                     args.epsilon,
                                                                                                     args.decay,
    ...
    final_log_file = "../outputs/2way-single-roundabout_{}/log/{}-metrics-train.log".format(EXPERIMENT_NO,
                                                                                             EXPERIMENT_NO)
    ...
    #seed = 70
    seed = args.seed
    print("Run: {} for Training experiment: {} - route: {} - first seed: {} - alpha: {} - gamma: {} - epsilon: {} - reward: {}".format(args.run_no, EXPERIMENT_NO, args.route, seed, args.alpha, args.gamma, args.epsilon, args.reward))
    logging.info("Run: {} for Training experiment: {} - route: {} - first seed: {} - alpha: {} - gamma: {} - epsilon: {} - reward: {}".format(args.run_no, EXPERIMENT_NO, args.route, seed, args.alpha, args.gamma, args.epsilon, args.reward))
    for run in range(1, args.runs+1):
        ...
        if run > 1:
            for agent_id in ql_agents.keys():
                ql_agents[agent_id].set_q_table(q_table[agent_id])
                #ql_agents[agent_id].set_current_epsilon(current_epsilon[agent_id])
        ...
        for agent_id in ql_agents.keys():
            q_table[agent_id] = ql_agents[agent_id].get_q_table()
            current_epsilon[agent_id] = ql_agents[agent_id].get_epsilon()
            #print("Run: {} - First Epsilon: {} - Last Epsilon: {}".format(run, args.epsilon, ql_agents[agent_id].get_epsilon()))
            logging.info("Run: {} - First Epsilon: {} - Last Epsilon: {}".format(run, args.epsilon, ql_agents[agent_id].get_epsilon()))
            #print("Run: {} - Agent Id: {} - Current Epsilon: {}".format(run, agent_id, current_epsilon[agent_id]))

        env.save_csv(out_csv, run)
        env.close()

    logging.info("Q-Table Total Distinct States: {}".format(len(q_table[ts].keys())))
    ...
    #print(f"Mean_Stopped={mean_stopped:.2f} +/- {std_stopped:.2f}")
    logging.info(f"Mean_Stopped={mean_stopped:.2f} +/- {std_stopped:.2f}")
    ...
    #print(f"Mean_waiting_time={mean_acc_wt:.2f} +/- {std_acc_wt:.2f}")
    logging.info(f"Mean_waiting_time={mean_acc_wt:.2f} +/- {std_acc_wt:.2f}")
    ...
    for index, elem in enumerate(all_per_episode_rewards):
        ...
        std_list.append(std)
        #print(f"Episode:{index} Mean Reward= {mean:.3f} +/- {std:.3f}")
        logging.info(f"Episode:{index} Mean Reward= {mean:.3f} +/- {std:.3f}")
    ...
    #print("****************************************")
    logging.info("****************************************")
    #draw2(list(), episode_rewards, chart_name='METRIC: Training Episode Rewards', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_list, chart_name='METRIC: Training Episode Rewards', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_stopped_list, chart_name='METRIC: Training Episode J0 Stopped', y_label="# of cars", draw_avg=False)
    ##draw2(list(), result_stopped_list, chart_name='METRIC: Training Episode J0 Stopped', y_label="# of cars", draw_avg=False)
    #draw2(list(), J0_stopped_cross_check_list, chart_name='METRIC: Training Episode J0 Stopped Cross Check', y_label="# of cars", draw_avg=False)
    ##draw2(list(), result_stopped_cross_check_list, chart_name='METRIC: Training Episode J0 Stopped Cross Check', y_label="# of cars", draw_avg=False)
    #draw2(list(), J0_acc_wt_list, chart_name='METRIC: Training Episode J0 Acc. Waiting Time', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_wt_list, chart_name='METRIC: Training Episode J0 Acc. Waiting Time', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_avg_speed_list, chart_name='METRIC: Training Episode J0 Avg. Speed', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_avg_speed_list, chart_name='METRIC: Training Episode J0 Avg. Speed', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_avg_speed_2_list, chart_name='METRIC: Training Episode J0 Avg. Speed 2', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_avg_speed_2_list, chart_name='METRIC: Training Episode J0 Avg. Speed 2', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_avg_speed_cross_check_list, chart_name='METRIC: Training Episode J0 Avg. Speed Cross Check', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_avg_speed_cross_check_list, chart_name='METRIC: Training Episode J0 Avg. Speed Cross Check', y_label="Seconds", draw_avg=False)

    #draw2(list(), J0_acc_average_co2_emission_tl_list, chart_name='METRIC: Training Episode J0 Avg. CO2 Emission TL', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_average_co2_emission_tl_list, chart_name='METRIC: Training Episode J0 Avg. CO2 Emission TL', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_average_co_emission_tl_list, chart_name='METRIC: Training Episode J0 Avg. CO Emission TL', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_average_co_emission_tl_list, chart_name='METRIC: Training Episode J0 Avg. CO Emission TL', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_average_fuel_consumption_tl_list, chart_name='METRIC: Training Episode J0 Avg. Fuel Consumption TL', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_average_fuel_consumption_tl_list, chart_name='METRIC: Training Episode J0 Avg. Fuel Consumption TL', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_average_noise_emission_tl_list, chart_name='METRIC: Training Episode J0 Avg. Noise Emission TL', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_average_noise_emission_tl_list, chart_name='METRIC: Training Episode J0 Avg. Noise Emission TL', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_average_co2_emission_list, chart_name='METRIC: Training Episode J0 Avg. CO2 Emission', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_average_co2_emission_list, chart_name='METRIC: Training Episode J0 Avg. CO2 Emission', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_average_co_emission_list, chart_name='METRIC: Training Episode J0 Avg. CO Emission', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_average_co_emission_list, chart_name='METRIC: Training Episode J0 Avg. CO Emission', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_average_fuel_consumption_list, chart_name='METRIC: Training Episode J0 Avg. Fuel Consumption', y_label="Seconds", draw_avg=False)
    ##draw2(list(), result_average_fuel_consumption_list, chart_name='METRIC: Training Episode J0 Avg. Fuel Consumption', y_label="Seconds", draw_avg=False)
    #draw2(list(), J0_acc_average_noise_emission_list, chart_name='METRIC: Training Episode J0 Avg. Noise Emission', y_label="Seconds", draw_avg=False)
