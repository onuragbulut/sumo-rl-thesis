...
class SumoEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        if self.single_agent:
            info.update({'reward': rewards[self.ts_ids[0]]})
    ...
    def _get_per_agent_info(self):
        vehicles = self.sumo.vehicle.getIDList()
        speeds = [self.sumo.vehicle.getSpeed(vehicle) for vehicle in vehicles]
        co2_emissions = [self.sumo.vehicle.getCO2Emission(vehicle) for vehicle in vehicles]
        co_emissions = [self.sumo.vehicle.getCOEmission(vehicle) for vehicle in vehicles]
        fuel_consumptions = [self.sumo.vehicle.getFuelConsumption(vehicle) for vehicle in vehicles]
        noise_emissions = [self.sumo.vehicle.getNoiseEmission(vehicle) for vehicle in vehicles]
        ...
        average_speed2 = [self.traffic_signals[ts].get_average_speed2() for ts in self.ts_ids]

        average_co2_emission = [self.traffic_signals[ts].get_average_co2_emission() for ts in self.ts_ids]
        average_co_emission = [self.traffic_signals[ts].get_average_co_emission() for ts in self.ts_ids]
        average_fuel_consumption = [self.traffic_signals[ts].get_average_fuel_consumption() for ts in self.ts_ids]
        average_noise_emission = [self.traffic_signals[ts].get_average_noise_emission() for ts in self.ts_ids]
        info = {}
        for i, ts in enumerate(self.ts_ids):
            info[f'{ts}_stopped'] = stopped[i]
            info[f'{ts}_stopped_cross_check'] = sum(int(speed < 0.1) for speed in speeds)
            info[f'{ts}_accumulated_waiting_time'] = accumulated_waiting_time[i]
            info[f'{ts}_average_speed'] = average_speed[i]
            info[f'{ts}_average_speed_2'] = average_speed2[i]
            info[f'{ts}_average_speed_cross_check'] = 0.0 if len(vehicles) == 0 else np.mean(speeds)

            info[f'{ts}_average_co2_emission_tl'] = average_co2_emission[i]
            info[f'{ts}_average_co_emission_tl'] = average_co_emission[i]
            info[f'{ts}_average_fuel_consumption_tl'] = average_fuel_consumption[i]
            info[f'{ts}_average_noise_emission_tl'] = average_noise_emission[i]

            info[f'{ts}_average_co2_emission'] = 0.0 if len(vehicles) == 0 else np.mean(co2_emissions)
            info[f'{ts}_average_co_emission'] = 0.0 if len(vehicles) == 0 else np.mean(co_emissions)
            info[f'{ts}_average_fuel_consumptions'] = 0.0 if len(vehicles) == 0 else np.mean(fuel_consumptions)
            info[f'{ts}_average_noise_emissions'] = 0.0 if len(vehicles) == 0 else np.mean(noise_emissions)
    ...
    def encode(self, state, ts_id):
        ...
        #density_queue = [self._discretize_density(d) for d in state[self.traffic_signals[ts_id].num_green_phases + 0:]]
    ...
    def _discretize_density(self, density):
        ...
        #return min(int(density * 5), 4)
        # ceil((density / 3) * 10)
        #2.67 => 0.89 => 8.9 => 9
        # ceil((density * 10) / 3)
        #2.67 => 26.7 => => 8.9 => 9
        #value = math.ceil((density*10) / 3)
        value = math.ceil(density * 10)
        return min(value, 9)
