...
class TrafficSignal:
    ...
    def __init__(self, 
        ...
        '''
        self.lane_index_dict = {
            '4i_0': 0, 'E0_0': 0, 'E0_1': 0,
            '2i_0': 1, 'E1_0': 1, 'E1_1': 1,
            '3i_0': 2, 'E2_0': 2, 'E2_1': 2,
            'E3_0': 3, 'E3_1': 3, '1i_0': 3
        }
        '''
        self.lane_index_dict = {
            '4i_0': 0,
    ...
    def _average_speed_reward2(self):
        return self.get_average_speed2()
    ...
    def _observation_fn_default(self):
        ...
        #observation = np.array(phase_id + density, dtype=np.float32)
    ...
    def get_average_co2_emission(self):
        avg_co2_emission = 0.0
        vehs = self._get_veh_list()
        if len(vehs) == 0:
            return 0.0
        for v in vehs:
            avg_co2_emission += self.sumo.vehicle.getCO2Emission(v)
        return avg_co2_emission / len(vehs)

    def get_average_co_emission(self):
        avg_co_emission = 0.0
        vehs = self._get_veh_list()
        if len(vehs) == 0:
            return 0.0
        for v in vehs:
            avg_co_emission += self.sumo.vehicle.getCOEmission(v)
        return avg_co_emission / len(vehs)
    ...
    def get_average_fuel_consumption(self):
        ...
        if len(vehs) == 0:
            return 0.0
        for v in vehs:
            avg_fuel_consumption += self.sumo.vehicle.getFuelConsumption(v)
        return avg_fuel_consumption / len(vehs)

    def get_average_noise_emission(self):
        avg_noise_emission = 0.0
        vehs = self._get_veh_list()
        if len(vehs) == 0:
            return 0.0
        for v in vehs:
            avg_noise_emission += self.sumo.vehicle.getNoiseEmission(v)
        return avg_noise_emission / len(vehs)
    ...
    def get_lanes_density(self):
        ...
        lanes_density_dict = {lane: self.sumo.lane.getLastStepVehicleNumber(lane) / (self.lanes_lenght[lane] / (self.MIN_GAP + self.sumo.lane.getLastStepLength(lane))) for lane in self.lanes}
        lanes_density_new = [0.0, 0.0, 0.0, 0.0]
        for key in lanes_density_dict.keys():
            if key in self.lane_index_dict.keys():
                index = self.lane_index_dict[key]
                value = lanes_density_dict[key]
                lanes_density_new[index] += value
                #print("{}: {}".format(key, lanes_density_dict[key]))

        #print("{}".format(lanes_density_new))
        #return [min(1, density) for density in lanes_density]
        lanes_density_new_avg = [density for density in lanes_density_new]
        return [min(1, density) for density in lanes_density_new_avg]
        #return [min(3, density) for density in lanes_density_new_avg]
    ...
    def get_lanes_queue(self):
        ...
        lanes_queue_dict = {lane: self.sumo.lane.getLastStepHaltingNumber(lane) / (self.lanes_lenght[lane] / (self.MIN_GAP + self.sumo.lane.getLastStepLength(lane))) for lane in self.lanes}
        lanes_queue_new = [0.0, 0.0, 0.0, 0.0]
        for key in lanes_queue_dict.keys():
            if key in self.lane_index_dict.keys():
                index = self.lane_index_dict[key]
                value = lanes_queue_dict[key]
                lanes_queue_new[index] += value
                #print("{}: {}".format(key, lanes_density_dict[key]))

        # print("{}".format(lanes_density_new))
        # return [min(1, density) for density in lanes_density]
        lanes_queue_new_avg = [density for density in lanes_queue_new]
        return [min(1, density) for density in lanes_queue_new_avg]
    ...
    reward_fns = {
        ...
        'average-speed-2': _average_speed_reward2,
        'queue': _queue_reward,
